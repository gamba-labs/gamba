...
import { ActionBar, Button, GambaLogo, ResponsiveSize } from 'gamba/react-ui'
    ...
    <>
      <ResponsiveSize>
        <Container>
          ...
          <Card key={cards.length}>
            ...
            <div className="suit"><GambaLogo /></div>
          </Card>
      ...
      <ActionBar>
        ...
        <Button loading={claiming} disabled={newSession || claiming || loading} onClick={() => claim()}>
          CLAIM {parseFloat(lamportsToSol(gamba.balances.user).toFixed(3))} SOL
        </Button>
      </ActionBar>
    </>
  )
