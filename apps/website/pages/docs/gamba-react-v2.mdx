...
function App() {
  return (
    <SendTransactionProvider priorityFee={10} simulationUnits={1000} computeUnitLimitMargin={200}>
...
const gambaProviderProps = {
  __experimental_plugins: [/* array of plugins */]
};
```
</Callout>
...
```jsx copy
const { balance, nativeBalance } = useBalance(new PublicKey('wallet-address'), new PublicKey('token-address'));
```
</Callout>
...
```jsx copy
const gambaEvents = useGambaEvents('GameSettled', { address: new PublicKey('event-address') });
```
</Callout>
...
```jsx copy
const poolData = usePool(new PublicKey('token-address'), new PublicKey('authority-address'));
```
</Callout>
