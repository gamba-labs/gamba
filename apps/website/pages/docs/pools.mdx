...
Upon creation of a pool for a specific token (referred to herein as the pool's "underlying token"), a corresponding LP token is generated. These LP tokens are minted through deposits, with initial deposits establishing an LP token ratio of 1:1 to the underlying token. However, this ratio dynamically adjusts over time as player activity influences the pool's total liquidity.
